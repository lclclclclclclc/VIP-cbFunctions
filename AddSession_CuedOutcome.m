...
path_to_session_directory = '';
CuedEventsFN = ''; % proto-TrialEvents struct supplied to MakeTrialEvents
...
returndir = pwd;
cd(path_to_session_directory);

    if ~exist('behavEVENTS.mat', 2)
...
MakeTrialEvents(cells{1}, CuedEvents, Events_TTL, Events_TS);
% okay to do with only cells{1} because all from same session

%% Define EventsEpochs function
% [events,epochs] = defineEventsEpochs_CuedOutcome; %for demo, no need to call it extra in general

%% Prealign the spikes
prealignSpikes(cells,'FUNdefineEventsEpochs',@defineEventsEpochs_CuedOutcome,'filetype','event','events',[],'epochs',[],'writing_behavior','overwrite','ifsave',1);
